#
%define nginx_home %{_localstatedir}/cache/nginx-quic
...
%define WITH_CC_OPT -I./boringssl/include
%define WITH_LD_OPT -L./boringssl/build/ssl -L./boringssl/build/crypto

%define BASE_CONFIGURE_ARGS $(echo "--prefix=%{_sysconfdir}/nginx-quic --sbin-path=%{_sbindir}/nginx-quic --modules-path=%{_libdir}/nginx-quic/modules --conf-path=%{_sysconfdir}/nginx-quic/nginx.conf --error-log-path=%{_localstatedir}/log/nginx-quic/error.log --http-log-path=%{_localstatedir}/log/nginx-quic/access.log --pid-path=%{_localstatedir}/run/nginx-quic.pid --lock-path=%{_localstatedir}/run/nginx-quic.lock --http-client-body-temp-path=%{_localstatedir}/cache/nginx-quic/client_temp --http-proxy-temp-path=%{_localstatedir}/cache/nginx-quic/proxy_temp --http-fastcgi-temp-path=%{_localstatedir}/cache/nginx-quic/fastcgi_temp --http-uwsgi-temp-path=%{_localstatedir}/cache/nginx-quic/uwsgi_temp --http-scgi-temp-path=%{_localstatedir}/cache/nginx-quic/scgi_temp --user=%{nginx_user} --group=%{nginx_group} --with-compat --with-file-aio --with-threads --with-http_addition_module --with-http_auth_request_module --with-http_dav_module --with-http_flv_module --with-http_gunzip_module --with-http_gzip_static_module --with-http_mp4_module --with-http_random_index_module --with-http_realip_module --with-http_secure_link_module --with-http_slice_module --with-http_ssl_module --with-http_stub_status_module --with-http_sub_module --with-http_v2_module --with-mail --with-mail_ssl_module --with-stream --with-stream_realip_module --with-stream_ssl_module --with-stream_ssl_preread_module")
...
Source0: https://github.com/kuretru/nginx/raw/master/%{name}-%{version}.tar.gz
Source1: logrotate
...
Source8: nginx-quic.service
Source9: nginx.upgrade.sh
Source10: nginx.suse.logrotate
Source11: nginx-quic-debug.service
Source12: COPYRIGHT
Source13: nginx.check-reload.sh
Source14: https://github.com/google/boringssl/archive/master.zip
...
sed -e 's|%%DEFAULTSTART%%|2 3 4 5|g' -e 's|%%DEFAULTSTOP%%|0 1 6|g' \
    -e 's|%%PROVIDES%%|nginx-quic|g' < %{SOURCE2} > nginx.init
sed -e 's|%%DEFAULTSTART%%||g' -e 's|%%DEFAULTSTOP%%|0 1 2 3 4 5 6|g' \
    -e 's|%%PROVIDES%%|nginx-quic-debug|g' < %{SOURCE2} > nginx-debug.init
unzip -q %{SOURCE14}
...
./configure %{BASE_CONFIGURE_ARGS} \
    ...
    --with-debug
make %{?_smp_mflags}
%{__mv} %{bdir}/objs/nginx-quic \
    %{bdir}/objs/nginx-quic-debug
./configure %{BASE_CONFIGURE_ARGS} \
    --with-http_v3_module \
    --with-openssl=./boringssl-master/ \
    --with-cc-opt="%{WITH_CC_OPT}" \
...
%{__mkdir} -p $RPM_BUILD_ROOT%{_datadir}/nginx-quic
%{__mv} $RPM_BUILD_ROOT%{_sysconfdir}/nginx-quic/html $RPM_BUILD_ROOT%{_datadir}/nginx-quic/

%{__rm} -f $RPM_BUILD_ROOT%{_sysconfdir}/nginx-quic/*.default
%{__rm} -f $RPM_BUILD_ROOT%{_sysconfdir}/nginx-quic/fastcgi.conf

%{__mkdir} -p $RPM_BUILD_ROOT%{_localstatedir}/log/nginx-quic
%{__mkdir} -p $RPM_BUILD_ROOT%{_localstatedir}/run/nginx-quic
%{__mkdir} -p $RPM_BUILD_ROOT%{_localstatedir}/cache/nginx-quic

%{__mkdir} -p $RPM_BUILD_ROOT%{_libdir}/nginx-quic/modules
cd $RPM_BUILD_ROOT%{_sysconfdir}/nginx-quic && \
    %{__ln_s} ../..%{_libdir}/nginx-quic/modules modules && cd -
...
%{__mkdir} -p $RPM_BUILD_ROOT%{_sysconfdir}/nginx-quic/conf.d
%{__rm} $RPM_BUILD_ROOT%{_sysconfdir}/nginx-quic/nginx.conf
%{__install} -m 644 -p %{SOURCE4} \
    $RPM_BUILD_ROOT%{_sysconfdir}/nginx-quic/nginx.conf
%{__install} -m 644 -p %{SOURCE5} \
    $RPM_BUILD_ROOT%{_sysconfdir}/nginx-quic/conf.d/default.conf

%{__mkdir} -p $RPM_BUILD_ROOT%{_sysconfdir}/sysconfig
%{__install} -m 644 -p %{SOURCE3} \
    $RPM_BUILD_ROOT%{_sysconfdir}/sysconfig/nginx-quic
%{__install} -m 644 -p %{SOURCE7} \
    $RPM_BUILD_ROOT%{_sysconfdir}/sysconfig/nginx-quic-debug

%{__install} -p -D -m 0644 %{bdir}/objs/nginx-quic.8 \
    $RPM_BUILD_ROOT%{_mandir}/man8/nginx-quic.8

%if %{use_systemd}
# install systemd-specific files
%{__mkdir} -p $RPM_BUILD_ROOT%{_unitdir}
%{__install} -m644 %SOURCE8 \
    $RPM_BUILD_ROOT%{_unitdir}/nginx-quic.service
%{__install} -m644 %SOURCE11 \
    $RPM_BUILD_ROOT%{_unitdir}/nginx-quic-debug.service
%{__mkdir} -p $RPM_BUILD_ROOT%{_libexecdir}/initscripts/legacy-actions/nginx-quic
%{__install} -m755 %SOURCE9 \
    $RPM_BUILD_ROOT%{_libexecdir}/initscripts/legacy-actions/nginx-quic/upgrade
%{__install} -m755 %SOURCE13 \
    $RPM_BUILD_ROOT%{_libexecdir}/initscripts/legacy-actions/nginx-quic/check-reload
%else
# install SYSV init stuff
%{__mkdir} -p $RPM_BUILD_ROOT%{_initrddir}
%{__install} -m755 nginx-quic.init $RPM_BUILD_ROOT%{_initrddir}/nginx-quic
%{__install} -m755 nginx-quic-debug.init $RPM_BUILD_ROOT%{_initrddir}/nginx-quic-debug
%endif
...
%{__install} -m 644 -p %{SOURCE10} \
    $RPM_BUILD_ROOT%{_sysconfdir}/logrotate.d/nginx-quic
%else
%{__install} -m 644 -p %{SOURCE1} \
    $RPM_BUILD_ROOT%{_sysconfdir}/logrotate.d/nginx-quic
%endif

%{__install} -m755 %{bdir}/objs/nginx-quic-debug \
    $RPM_BUILD_ROOT%{_sbindir}/nginx-quic-debug
...
%{_sbindir}/nginx-quic
%{_sbindir}/nginx-quic-debug

%dir %{_sysconfdir}/nginx-quic
%dir %{_sysconfdir}/nginx-quic/conf.d
%{_sysconfdir}/nginx-quic/modules

%config(noreplace) %{_sysconfdir}/nginx-quic/nginx.conf
%config(noreplace) %{_sysconfdir}/nginx-quic/conf.d/default.conf
%config(noreplace) %{_sysconfdir}/nginx-quic/mime.types
%config(noreplace) %{_sysconfdir}/nginx-quic/fastcgi_params
%config(noreplace) %{_sysconfdir}/nginx-quic/scgi_params
%config(noreplace) %{_sysconfdir}/nginx-quic/uwsgi_params
%config(noreplace) %{_sysconfdir}/nginx-quic/koi-utf
%config(noreplace) %{_sysconfdir}/nginx-quic/koi-win
%config(noreplace) %{_sysconfdir}/nginx-quic/win-utf

%config(noreplace) %{_sysconfdir}/logrotate.d/nginx-quic
%config(noreplace) %{_sysconfdir}/sysconfig/nginx-quic
%config(noreplace) %{_sysconfdir}/sysconfig/nginx-quic-debug
%if %{use_systemd}
%{_unitdir}/nginx-quic.service
%{_unitdir}/nginx-quic-debug.service
%dir %{_libexecdir}/initscripts/legacy-actions/nginx-quic
%{_libexecdir}/initscripts/legacy-actions/nginx-quic/*
%else
%{_initrddir}/nginx-quic
%{_initrddir}/nginx-quic-debug
%endif

%attr(0755,root,root) %dir %{_libdir}/nginx-quic
%attr(0755,root,root) %dir %{_libdir}/nginx-quic/modules
%dir %{_datadir}/nginx-quic
%dir %{_datadir}/nginx-quic/html
%{_datadir}/nginx-quic/html/*

%attr(0755,root,root) %dir %{_localstatedir}/cache/nginx-quic
%attr(0755,root,root) %dir %{_localstatedir}/log/nginx-quic

%dir %{_datadir}/doc/%{name}-%{main_version}
%doc %{_datadir}/doc/%{name}-%{main_version}/COPYRIGHT
%{_mandir}/man8/nginx-quic.8*
...
if [ $1 -eq 1 ]; then
%if %{use_systemd}
    /usr/bin/systemctl preset nginx-quic.service >/dev/null 2>&1 ||:
    /usr/bin/systemctl preset nginx-quic-debug.service >/dev/null 2>&1 ||:
%else
    /sbin/chkconfig --add nginx-quic
    /sbin/chkconfig --add nginx-quic-debug
%endif
    # print site info
    cat <<BANNER
----------------------------------------------------------------------
...
BANNER
    ...
    if [ -d %{_localstatedir}/log/nginx-quic ]; then
        if [ ! -e %{_localstatedir}/log/nginx-quic/access.log ]; then
            touch %{_localstatedir}/log/nginx-quic/access.log
            %{__chmod} 640 %{_localstatedir}/log/nginx-quic/access.log
            %{__chown} nginx:%{nginx_loggroup} %{_localstatedir}/log/nginx-quic/access.log
        fi

        if [ ! -e %{_localstatedir}/log/nginx-quic/error.log ]; then
            touch %{_localstatedir}/log/nginx-quic/error.log
            %{__chmod} 640 %{_localstatedir}/log/nginx-quic/error.log
            %{__chown} nginx:%{nginx_loggroup} %{_localstatedir}/log/nginx-quic/error.log
        fi
    fi
fi
...
if [ $1 -eq 0 ]; then
%if %use_systemd
    /usr/bin/systemctl --no-reload disable nginx-quic.service >/dev/null 2>&1 ||:
    /usr/bin/systemctl stop nginx-quic.service >/dev/null 2>&1 ||:
%else
    /sbin/service nginx-quic stop > /dev/null 2>&1
    /sbin/chkconfig --del nginx-quic
    /sbin/chkconfig --del nginx-quic-debug
%endif
fi
...
%endif
if [ $1 -ge 1 ]; then
    /sbin/service nginx-quic status  >/dev/null 2>&1 || exit 0
    /sbin/service nginx-quic upgrade >/dev/null 2>&1 || echo \
